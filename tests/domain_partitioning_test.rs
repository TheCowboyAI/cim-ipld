...
use cim_ipld::object_store::{
    ContentDomain, PartitionStrategy, PatternMatcher,
};
...
fn test_music_domain_detection() {
    ...
    for ext in &["wav", "flac", "ogg", "m4a", "aac"] {
        ...
        assert_eq!(domain, ContentDomain::Music, "Failed for extension: {}", ext);
    }
}
...
fn test_configuration_detection() {
    ...
    for ext in &["json", "yaml", "toml", "ini", "conf", "xml"] {
        ...
        assert_eq!(domain, ContentDomain::Configuration, "Failed for extension: {}", ext);
    }
}
...
fn test_pattern_priority() {
    ...
    let content = "This contract for services includes Invoice Number: INV-001 with payment due in 30 days";
    ...
    let domain2 = strategy.determine_domain(
        Some("document.pdf"),
        None,
        Some(equal_match_content),
        None,
    );
...
fn test_bucket_name_mapping() {
    ...
    assert_eq!(strategy.get_bucket_for_domain(ContentDomain::Invoices), "cim-finance-invoices");
    assert_eq!(strategy.get_bucket_for_domain(ContentDomain::Memes), "cim-social-memes");
    assert_eq!(strategy.get_bucket_for_domain(ContentDomain::Medical), "cim-health-medical");
    assert_eq!(strategy.get_bucket_for_domain(ContentDomain::SourceCode), "cim-tech-code");
}
