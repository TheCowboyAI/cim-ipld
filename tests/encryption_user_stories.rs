...
async fn test_basic_encryption_with_cid_preservation() {
    use cim_ipld::encryption::{EncryptionType, EncryptedContent, EncryptionKey};
    ...
    assert_eq!(encrypted.metadata.encryption_type, EncryptionType::AES256_GCM);
...
async fn test_searchable_encryption() {
    ...
    for (name, content) in items {
        ...
        let encrypted_cid = encrypted_store.store_encrypted(
            content,
            &key,
            EncryptionType::AES256_GCM,
        ).await.unwrap();
    ...
    let result = encrypted_store.find_by_original_cid(search_cid).await.unwrap();
...
async fn test_key_management() {
    use cim_ipld::encryption::{
        KeyManager, KeyHierarchy, AccessPolicy,
        UserKey, GroupKey, DataEncryptionKey,
    };
    ...
    // Add user to group
    key_manager.add_user_to_group(user_id, group_id).await.unwrap();
    ...
    let can_access = key_manager.can_user_access(user_id, &policy.content_cid).await.unwrap();
    assert!(can_access);
    
    // And: User can decrypt DEK
    let user_group_key = key_manager.get_group_key_for_user(user_id, group_id).await.unwrap();
...
async fn test_selective_processing() {
    ...
    let result = processor.transform_encrypted(
        encrypted,
    ...
    assert_eq!(result.metadata.encryption_type, EncryptionType::ChaCha20Poly1305);
...
async fn test_encryption_migration() {
    use cim_ipld::encryption::{
        MigrationService, MigrationConfig, MigrationProgress,
        EncryptionType,
    };
    ...
    let progress_tracker = migration.start_migration(
        old_key.clone(),
        new_key.clone(),
    ).await.unwrap();
...
async fn test_zero_knowledge_search() {
    use cim_ipld::encryption::{
        ZKSearchIndex, SearchToken, BlindedQuery,
    ...
    let documents = vec![
        ("doc1", "Rust programming language", vec!["rust", "programming", "systems"]),
        ("doc2", "Go programming language", vec!["go", "programming", "google"]),
    ...
    for (id, content, keywords) in documents {
        ...
        for keyword in keywords {
            ...
            index.add_to_index(token, encrypted.calculate_cid().unwrap()).await.unwrap();
        }
    }
