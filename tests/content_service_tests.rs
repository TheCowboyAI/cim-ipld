...
use cim_ipld::{ContentType, TypedContent, Cid, Error};
...
fn test_content_type_restrictions() {
    ...
    assert!(validate_content_type(&ContentType::Markdown, &allowed_types));
    assert!(validate_content_type(&ContentType::Json, &allowed_types));
    assert!(!validate_content_type(&ContentType::Command, &allowed_types));
...
fn test_lifecycle_hooks() {
    ...
    let test_cid = Cid::try_from("bafybeigdyrzt5sfp7udm7hu76uh7y26nf3efuylqabf3oclgtqy55fbzdi").unwrap();
    post_store(&test_cid);
...
fn test_search_integration() {
    ...
    // Calculate CIDs
    let cids: Vec<Cid> = docs.iter()
        .map(|d| d.calculate_cid().unwrap())
        .collect();
...
fn test_batch_operations() {
    ...
    let cids: Result<Vec<Cid>, Error> = docs.iter()
        .map(|d| d.calculate_cid())
        .collect();
...
fn test_list_by_content_type() {
    ...
    let json_cid = Cid::try_from("bafybeigdyrzt5sfp7udm7hu76uh7y26nf3efuylqabf3oclgtqy55fbzdi").unwrap();
    ...
    // Filter by type
    let markdown_items: Vec<_> = content_map.iter()
